...
def rrt(map_obj, start_point, end_point, max_iterations=100000):
    x_range = map_obj.x_range
    y_range = map_obj.y_range
    obstacles = map_obj.obstacles
    ...
    for _ in range(max_iterations):
        ...
        if nearest_dist >= 0.5 and nearest_dist <= 1.0 and is_far_from_obstacles(target_point, obstacles):
            ...
            parent_nodes[target_point] = nearest_parent
            draw_map(map_obj, nodes)
    
    # Po zakończeniu 10000 iteracji, sprawdzamy, czy znaleziono ścieżkę
    if end_point in parent_nodes:
        print("Path found.")
        path = [end_point]
