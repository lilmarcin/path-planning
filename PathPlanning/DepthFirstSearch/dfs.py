...
def dfs(ax, map_obj, start_point, end_point, only_result):
    """
    ...
    stack = [(start_point, [])]  # Stack with start_point and path to point
    plt.ion()
    visited = set()
    visited_points = set()
    ...
    while stack:
        ...
        for neighbor in valid_neighbors:
            if neighbor not in visited:
                stack.append((neighbor, path + [current_point]))
    plt.ioff()
    plt.show()
